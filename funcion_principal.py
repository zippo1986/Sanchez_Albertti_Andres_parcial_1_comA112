...
def main ():
    ...
    while confirmacion == "s":
        ...
        match (opcion):
            case "1":
                lista_peliculas= archivo_a_lista("movies.csv")
            ...
            case "5":
                if not bandera_genero:
                    mostrar_mensaje("No se puede listar sin asignar antes el genero")
                else:
                    genero = input("Ingrese genero a filtrar")
                    genero =genero.lower()
                    while not validar_genero(genero):
                        genero = input("Ingrese genero a filtrar")
                    lista_filtrada = filtrar_datos(lista_peliculas, genero, "genero")
                    guardar_csv(lista_filtrada,f"{genero}.csv")
            case "6":
            ...
            case "7":
                if not bandera_rating:
                    ...
                    peliculas_max_rating = calcular_maximo(lista_peliculas,"rating")
            ...
            case "8":
                if bandera_peliculas_max_rat:
                    guardar_json(peliculas_max_rating, "peliculas_max_rat.json")
